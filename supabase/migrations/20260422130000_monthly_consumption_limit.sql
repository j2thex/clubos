-- Monthly consumption limit (grams) — club-wide.
--
-- NULL means "no limit; disabled". Non-NULL is a hard cap in grams that
-- record_sale enforces per member, per calendar month, counting only
-- products with unit = 'gram' and excluding voided transactions.

ALTER TABLE clubs
  ADD COLUMN IF NOT EXISTS monthly_consumption_limit_grams numeric(10,3);

-- Replace record_sale with the enforcement block. Otherwise identical to
-- 20260422120500_record_sale_rpc.sql.
--
-- Over-limit error is encoded as `over_consumption_limit:<used>:<limit>:<attempted>`
-- so the client can render remaining allowance without a second round trip.
...
DECLARE
  ...
  v_cart_grams numeric(10,3) := 0;
  v_month_grams numeric(10,3);
BEGIN
  ...
  SELECT currency_mode, monthly_consumption_limit_grams
    INTO v_currency_mode, v_monthly_limit
  ...
  FOR v_line IN SELECT * FROM jsonb_array_elements(p_lines) LOOP
    ...
    IF v_product.unit = 'gram' THEN
      v_cart_grams := v_cart_grams + v_quantity;
    END IF;
  END LOOP;

  -- Monthly consumption cap (grams, excluding voided, current calendar month UTC).
  IF v_monthly_limit IS NOT NULL AND v_cart_grams > 0 THEN
    SELECT COALESCE(SUM(pt.quantity), 0) INTO v_month_grams
    FROM product_transactions pt
    JOIN products p ON p.id = pt.product_id
    WHERE pt.club_id = p_club_id
      AND pt.member_id = p_member_id
      AND pt.voided_at IS NULL
      AND p.unit = 'gram'
      AND pt.created_at >= date_trunc('month', now() AT TIME ZONE 'UTC');

    IF v_month_grams + v_cart_grams > v_monthly_limit THEN
      RAISE EXCEPTION
        'over_consumption_limit:%:%:%',
        v_month_grams, v_monthly_limit, v_cart_grams
        USING ERRCODE = 'P0001';
    END IF;
  END IF;
