-- record_sale v2: scale tolerance enforcement + audit fields.
--
-- Adds two optional per-line fields to p_lines:
--   weight_requested numeric  -- target grams the staff committed to before
--     weighing (or computed from a price-mode entry). NULL means no target.
--   price_input numeric       -- € amount entered by staff in price mode.
--     NULL for weight mode.
--
-- Defence-in-depth: when weight_source = 'scale' AND weight_requested IS NOT
-- NULL, the server re-checks |quantity - weight_requested| ≤ 0.05 and raises
-- weight_outside_tolerance. The UI guard already prevents this; the server
-- check makes a forged client payload safe.
--
-- The parent sale row also gets scale_used = true if any line in the sale is
-- scale-sourced. The admin transactions page filters on this denormalised
-- column, avoiding a per-row aggregate join.
...
DECLARE
  ...
  v_weight_requested numeric;
  v_price_input numeric;
  v_scale_raw text;
  v_product products%ROWTYPE;
  v_line_total numeric(10,2);
  v_line_count integer := 0;
  v_any_scale boolean := false;
BEGIN
  -- 1. Input shape
  ...
  FOR v_line IN SELECT * FROM jsonb_array_elements(p_lines) LOOP
    ...
    v_weight_requested := NULLIF(v_line ->> 'weight_requested', '')::numeric;
    ...
    -- Tolerance re-check: only when staff committed to a target AND used the
    -- scale. Manual entries are trust-based (recorded as 'manual') and
    -- price-mode without scale falls back to the computed target.
    IF v_weight_source = 'scale'
       AND v_weight_requested IS NOT NULL
       AND abs(v_quantity - v_weight_requested) > 0.05 THEN
      RAISE EXCEPTION 'weight_outside_tolerance' USING ERRCODE = 'P0001';
    END IF;
  ...
  -- 8. Insert parent sale (scale_used denormalised from line loop above)
  INSERT INTO sales (
    club_id, member_id, fulfilled_by,
    subtotal, discount, total,
    paid_with, comment, scale_used
  )
  VALUES (
  ...
  FOR v_line IN SELECT * FROM jsonb_array_elements(p_lines) LOOP
    ...
    v_weight_requested := NULLIF(v_line ->> 'weight_requested', '')::numeric;
    v_price_input := NULLIF(v_line ->> 'price_input', '')::numeric;
    ...
    VALUES (
      ...
      v_weight_requested, v_price_input
    );
  END LOOP;
  ...
  VALUES (
    ...
    v_member_code,
    format(
      '%s line(s) · subtotal %s € · discount %s € · total %s € · %s%s',
      v_line_count::text,
      v_subtotal::text,
      v_discount::text,
      v_total::text,
      p_paid_with,
      CASE WHEN v_any_scale THEN ' · scale' ELSE '' END
    )
  );
