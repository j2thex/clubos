...
export default async function PeoplePage({
  ...
  const [
    ...
    supabase
      .from("members")
      .select(
        "id, member_code, full_name, spin_balance, is_staff, status, can_do_entry, can_do_sell, can_do_topup, can_do_transactions, can_do_qebo, can_manage_products, can_manage_identity, member_roles(name)"
      )
      .eq("club_id", club.id)
      .eq("is_staff", true)
      .eq("is_system_member", false)
  ...
  const staffList = (staff ?? []).map((s) => ({
    ...
    canManageProducts: s.can_manage_products ?? true,
    canManageIdentity: s.can_manage_identity ?? true,
  }));
