...
export default async function ContentPage({
  ...
  const [{ count: eventCount }, { count: questCount }, { count: amenityCount }, { count: badgeCount }] =
    await Promise.all([
      supabase
        .from("events")
  ...
  const items = [
    ...
    {
      label: t(locale, "admin.contentAmenities"),
      href: `/${clubSlug}/admin/amenities`,
      count: amenityCount ?? 0,
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
